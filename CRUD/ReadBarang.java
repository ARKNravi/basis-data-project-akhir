...
public class ReadBarang {
        ...
        try {
            bacaStokBarang();
            // Call other methods for testing if needed
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }

    public static void bacaBarang(DefaultTableModel model) throws SQLException {
        try (Connection conn = SQLConnection.getConnection()) {
            Statement st = conn.createStatement();
            ResultSet rs = st.executeQuery("SELECT b.kode_barang, b.nama_barang, b.harga, COALESCE(SUM(sb.offlen + sb.shopee + sb.tokopedia), 0) AS total_stok " +
                    "FROM barang b " +
    ...
    public static void bacaStokBarang() throws SQLException {
        try (Connection conn = SQLConnection.getConnection()) {
            Statement st = conn.createStatement();
            ResultSet rs = st.executeQuery("SELECT b.kode_barang, b.nama_barang, SUM(sb.offlen + sb.shopee + sb.tokopedia) AS total_stok " +
                    "FROM barang b " +
