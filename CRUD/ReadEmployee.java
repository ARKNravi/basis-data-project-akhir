...
public class ReadEmployee {
    public static void bacaSemuaPegawai(Connection conn) throws SQLException {
        ...
        try (PreparedStatement preparedStatement = conn.prepareStatement(selectQuery);
            ...
            System.out.printf("%-15s %-15s %-30s %-15s %-15s %-20s %-15s %-7s %-20s %-20s %-4s %-4s %20s %20s\n",
                    "NIP", "NIK", "Name", "Jabatan", "Pendidikan", "Asal", "Tanggal Lahir", "Kelamin", "NPWP",
                    "Nomor Rekening", "Status Pegawai", "Cuti", "Izin", "Gaji Pokok", "Tunjangan");
            System.out.println("--------------------------------------------------");
            ...
            while (resultSet.next()) {
                ...
                System.out.printf("%-15s %-15s %-30s %-15s %-15s %-20s %-15s %-7s %-20s %-20s %-4s %-4s %20s %20s\n",
                        nip, nik, nama, jabatan, pendidikan, asal, tanggalLahir, kelamin, npwp, norek, status, cuti,
                        izin, gaji, tunjangan);
            }
        }
    ...
    public static ResultSet bacaSemuaPegawaiResultSet(Connection conn) {
        try {
            String selectQuery = "SELECT  dp.nip, p.nik, p.nama, j.nama_jabatan, p.pendidikan, p.tempat, p.tanggal_lahir, p.jenis_kelamin, p.npwp, p.nomor_rekening, dp.status_pegawai, dp.total_cuti, dp.total_izin, g.gaji, t.tunjangan\n"
                    +
                    "FROM detail_pegawai dp\n" +
